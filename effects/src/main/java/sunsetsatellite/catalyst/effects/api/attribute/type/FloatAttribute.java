			...
			for (Function<?, List<Modifier<?>>> modifierSupplier : target.getContainer().additionalModifierSuppliers) {
				...
				for (NumberModifier<? extends Number> modifier : modifiers) {
					switch (modifier.type){
						case SET: value = modifier.value.floatValue(); break;
						case ADD: value += modifier.value.floatValue(); break;
						case SUBTRACT: value -= modifier.value.floatValue(); break;
						case PERCENT_ADD: value += (value/100) * modifier.value.floatValue(); break;
						case PERCENT_SUBTRACT: value -= (value/100) * modifier.value.floatValue(); break;
			...
			for (EffectStack effectStack : sortedStacks) {
				if(effectStack.hasAttribute(this) && effectStack.isActive()){
					...
					for (NumberModifier<? extends Number> modifier : validModifiers) {
						switch (modifier.type){
							case SET: value = modifier.calculate(effectStack).floatValue(); break;
							...
							case PERCENT_ADD: value += (value/100) * modifier.calculate(effectStack).floatValue(); break;
							case PERCENT_SUBTRACT: value -= (value/100) * modifier.calculate(effectStack).floatValue(); break;
