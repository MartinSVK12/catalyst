...
public class BlockModelMultipart extends BlockModelStandard<BlockLogic> {
	...
	public BlockModelMultipart(Block<BlockLogic> block, ModernMultipartBlockModel model, boolean render3d, float renderScale) {
		super(block);
	...
	public MultipartInternalModel[] getModelsFromState(Block<?> block, int x, int y, int z, boolean sourceFromWorld) {
		...
		if (tile instanceof ISupportsMultiparts) {
			((ISupportsMultiparts) tile).getParts().forEach((dir, multipart) -> {
				...
				//Y = 180 -> SOUTH | Z POS
				//Y = 270 -> WEST | X NEG
				MultipartInternalModel internalModel = new MultipartInternalModel(MultipartModelHelper.getOrCreateBlockModel(CatalystMultipart.MOD_ID, multipart), dir.getSide(), multipart);
