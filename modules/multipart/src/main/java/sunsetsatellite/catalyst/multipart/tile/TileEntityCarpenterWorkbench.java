...
public class TileEntityCarpenterWorkbench extends TileEntity implements Container, IScreenActionListener {
	...
	public void tick() {
		...
		if (contents[0] != null && contents[0].itemID < 16384) {
			Block<?> block = Blocks.getBlock(contents[0].itemID);
			boolean valid = block.hasTag(CatalystMultipart.CAN_BE_MULTIPART) || block.isCubeShaped();
			if (contents[1] != null && contents[1].getItem() instanceof ItemToolAxe && valid) {
				MultipartType.types.forEach((K, V) -> {
					if (!block.hasTag(CatalystMultipart.TYPE_TAGS.get(K)) && !block.isCubeShaped()) return;
					...
					if (selectedSide != Side.NONE) {
						multipartTag.putInt("Side", selectedSide.id);
					}
	...
	public void writeAdditionalData(@NonNull CompoundTag nbttagcompound) {
		...
		for (int i = 0; i < this.contents.length; ++i) {
			if (this.contents[i] != null) {
				...
				list.addTag(nbttagcompound1);
			}
		}

		nbttagcompound.put("Items", list);
	}
