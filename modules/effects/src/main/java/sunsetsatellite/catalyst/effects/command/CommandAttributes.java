	...
	public static void listAttributes(CommandContext<CommandSource> ctx, IHasEffects<?> target) {
		...
		try {
			for (Attribute<?> attribute : Attributes.getInstance()) {
				List<Attribute<?>> list = attrByMod.computeIfAbsent(NamespaceID.fromPool(attrs.getKey(attribute)).namespace(), k -> new ArrayList<>());
				list.add(attribute);
			}
			...
			for (String key : attrByMod.keySet()) {
				...
				for (Attribute<?> attribute : attributes) {
					source.sendMessage(
						String.format(
							"- %s: %s",
							NamespaceID.fromPool(attrs.getKey(attribute)).value(),
