...
public class HeartContainer {
	...
	public String getPathForGlyph(HeartGlyphVariant glyphVariant, HeartGlyphType glyphType) {
		...
		String prefix = (glyphVariant != HeartGlyphVariant.NONE ? glyphVariant.name + "/" : "survival/");
		if( glyphVariant == HeartGlyphVariant.PREVIEW){
			prefix = "survival/"+prefix;
		}
		return getBasePath() + prefix + glyphType.name + (isHeartFlashing() ? "_blinking" : "");
	}

	public void drawHeart(HeartGlyphVariant variant, HeartGlyphType glyphType, int x, int y, Gui hud) {
