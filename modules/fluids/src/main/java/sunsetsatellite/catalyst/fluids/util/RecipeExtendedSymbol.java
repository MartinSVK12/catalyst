...
public class RecipeExtendedSymbol {
	...
	public boolean matches(ItemStack stack) {
		...
		for (ItemStack resolvedStack : stacks) {
			if (resolvedStack.itemID != stack.itemID) continue;
			if (resolvedStack.getMetadata() != -1 && resolvedStack.getMetadata() != stack.getMetadata()) continue;

			CompoundTag resolvedData = resolvedStack.getData();
			if (resolvedData.getValue().isEmpty()) return true;

			if (isNBTPresentAndEqual(resolvedData, stack.getData())) return true;
		}
		return false;
	}

	private boolean isNBTPresentAndEqual(CompoundTag requirement, CompoundTag tagToCompare) {
		for (String key : requirement.getValue().keySet()) {
			Tag<?> requirementTag = requirement.getTag(key);
			Tag<?> comparrisonTag = tagToCompare.getTag(key);

			if (comparrisonTag == null || !comparrisonTag.equals(requirementTag)) {
				return false;
			}
		}
		return true;
