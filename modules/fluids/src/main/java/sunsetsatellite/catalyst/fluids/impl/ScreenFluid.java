...
public class ScreenFluid extends ScreenContainerAbstract {
	...
	public void render(int mx, int my, float partialTick) {
		...
		if (slot != null && slot.hasStack()) {
			boolean showDescription = GameSettings.ITEM_DESCRIPTIONS.value == ALWAYS_SHOW || GameSettings.KEY_DESCRIPTION.isPressed();
	...
	public void clickFluidInventory(int mx, int my, int button) {
		...
		if (slotId != -1) {
			...
			if (GameSettings.SWAP_CRAFTING_BUTTONS.value) {
				boolean a = shift;
				shift = control;
				control = a;
			}
